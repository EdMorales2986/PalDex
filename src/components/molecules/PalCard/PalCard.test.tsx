...
describe("PalCard", () => {
  beforeEach(() => {
    render(<PalCard data={exData} onClick={onClick} />);
  });
  ...
  test("should render the image", () => {
    expect(screen.getByAltText(/bulbasaur/i)).toBeDefined();
    expect(screen.getByAltText(/bulbasaur/i)).toHaveProperty(
      "src",
      exData.sprites.front_default
    );
  });

  test("should render the types", () => {
    expect(screen.getByText(/grass/i)).toBeDefined();
    expect(screen.getByText(/poison/i)).toBeDefined();
  });

  test("if the card is clicked, the onClick function should be called", () => {
    fireEvent.click(screen.getByText(/bulbasaur/i));
    expect(onClick).toHaveBeenCalled();
  });

  test("if the image is undefined, the default image should be rendered", () => {
    cleanup();
    render(<PalCard data={{ ...exData, sprites: {} }} onClick={onClick} />);
    expect(screen.getByAltText(/bulbasaur/i)).toBeDefined();
  });
});
